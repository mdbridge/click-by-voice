...
(function() {
    ...
    class HintNumberGenerator {
        ...
        generate() {
            ...
            if (!Hints.option("avoiding_reuse") && this.#retired_hint_numbers.length > 0) {
                ...
                Util.vlog(4)(`reusing hint number ${number}`);
                return number;
            }
    ...
    class Hint {
        ...
        get hinted_element() {
            ...
            if (!element) {
                Util.vlog(4)(`The element that had hint ${this.#hint_number} longer exists`);
                return undefined;
            }
            if (!element.isConnected) {
                Util.vlog(4)(`The element with hint ${this.#hint_number} is no longer connected`);
                return undefined;
            }
            return element;
        }
        ...
        adjust() {
            ...
            if (!$inner[0].isConnected) {
                ...
                Batcher.mutating(() => {
                    Util.vlog(4)(`lost hint for ${this.#hint_number}; removing...`);
            ...
            if (element_hidden) {
                ...
                Batcher.mutating(() => {
                    Util.vlog(5)(`hiding hint for hidden element ${hint_number};` +
            ...
            if (inner_hidden) {
                ...
                Batcher.mutating(() => {
                    Util.vlog(5)(`unhiding hint for unhidden element ${hint_number}`);
            ...
            if (Math.abs(inner_box.left - target_left) > 0.5 ||
                ...
                let inner_left = parseFloat(style.left);
                Batcher.mutating(() => {
                    Util.vlog(5)(`(re)positioning overlay for ${hint_number}`);
                    Util.vlog(5)(`  ${inner_box.top} x ${inner_box.left}` +
    ...
    var _remove_hint = function _remove_hint(hint_number, hinted_element) {
        Util.vlog(5)(`removing ${hint_number}:`);
        Util.vlog(5)(hinted_element);
        hinted_elements.delete(hinted_element);
        hint_number_to_hint.delete(hint_number);
        if (Hints.option("mark_hinted")) {
            $(`[CBV_hint_number='${hint_number}']`).removeAttr("CBV_hint_number");
